        ...
        <el-table-column label="Thao tác" align="center">
          <template slot-scope="scope">
            <span v-if="scope.row.status == 'approved'">
              <button v-if="scope.row.currentProcessId != idProfile" style="margin-right: 20px" class="btn-primary" @click="publishDocument(scope.row)">Ban hành</button>       
            </span>
    ...
    methods: {
      ...
      publishDocument(data) {
        editDocument({
          ...
          this.getList()
          this.$message('Ban hành tài liệu thành công')
        })
      },
      rejectDocument() {
        editDocument({
