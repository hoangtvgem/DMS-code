        ...
        <el-table-column label="Thao tác" align="center">
          <template slot-scope="scope">
            <span v-if="scope.row.status == 'reject'">
              <button style="margin-right: 20px" class="btn-primary" @click="requestDocument(scope.row)">Yêu cầu duyệt lại</button>      
            </span>
  ...
  export default {
    ...
    methods: {
      ...
      approveDocument(data) {
        editDocument({
          ...data,
          currentProcessId: data.process[data.process.indexOf(this.idProfile) + 1]
        }).then(response => {
          this.getList()
