        ...
        <el-table-column label="Thao tác" align="center">
          <template slot-scope="scope">
            <span v-if="scope.row.status == 'pending'">
              <button v-if="scope.row.currentProcessId == idProfile && scope.row.owner != idProfile" style="margin-right: 20px" class="btn-primary" @click="approveDocument(scope.row)">Duyệt</button>   
              <button v-if="scope.row.currentProcessId == idProfile && scope.row.owner != idProfile" style="margin-right: 20px" class="btn-danger" @click="showModal(scope.row)">Từ chối</button>      
            </span>
